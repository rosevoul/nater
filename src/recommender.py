...
class Recommender(object):
    ...
    def preprocess_data(self):
        ...
        self.test_blocks_n_keywords = self.prep.preprocess_variable_names(
    ...
    def recommend_test_blocks(self, test_step_description, N=10, method='avg', text_weight=0.9, param_weight=0.1):
        return [self.data.test_blocks_names[i] for i in self.find_top_blocks(test_step_description, N, method, text_weight, param_weight)]
    ...
    def find_top_blocks(self, test_step_description, N, method, text_weight, param_weight):
        description_keywords, parameters = self.extract_step_keywords(
            test_step_description)
        # Similarity of keywords
        k_similarities = compute_similarities(
            description_keywords, self.test_blocks_nd_keywords, method, self.model)
        # Similarity of parameters
        p_similarities = parameter_similarities(
            parameters, self.test_blocks_parameters)
        # Recommender scores
        top_blocks_indices, self.blocks_scores = assign_scores(
            N, k_similarities, text_weight, p_similarities, param_weight)
    ...
    def find_top_reqs(self, test_scenario, N, method):
        ...
        top_req_indices, self.req_scores = most_similar_text(
            test_keywords, self.reqs_keywords, self.model, N, method)
...
class PhaseIIRecommender(Recommender):
    ...
    def parse_user_input(self, user_test_step):
        ...
        expected_result_sentences = self.prep.split_compound_sentence(
            expected_result)

        print("Description contains ", len(
            description_sentences), " sentences.")
        print("Expected result contains ", len(
            expected_result_sentences), " sentences.")
        ...
        if precondition_keywords:
            ...
            if max(pres_similarities) < threshold:
                ...
                middle_step_expected_result = self.data.test_blocks_preconditions[
                    bi]
                print("MIDDLE STEP EXPECTED RESULT: ",
                      middle_step_expected_result)
...
class RecommenderWithUserFeedback(Recommender):
    ...
    def extract_old_tests(self):
        ...
        for timestamp in timestamps:
            test = self.tests_dataframe.loc[self.tests_dataframe[
                'timestamp'] == timestamp, 'block_id'].tolist()
            tests.append(test)
    ...
    def store_test_n_blocks(self, title, steps, blocks):
        ...
        current_test = pd.DataFrame(
            {'step': steps, 'block': blocks, 'block_id': block_ids})
        current_test['timestamp'] = timestamp
        current_test['title'] = title
        self.tests_dataframe = self.tests_dataframe.append(
            current_test, ignore_index=True)

        self.tests_dataframe.to_csv(self.tests_blocks_file, index=False)
        self.start_scenario = 1
    ...
    def find_top_blocks(self, test_step_description, selected_blocks, N, method):
        ...
        if selected_blocks and old_tests:
            min_occurences = max(5, int(len(old_tests) / 2))
            if self.start_scenario:
                self.itemsets = dict(
                    frequent_itemsets(old_tests, min_occurences))
                self.start_scenario = 0
            confidence_scores = score_associated_blocks(
                self.itemsets, selected_blocks, test_blocks_indices)
        else:
            confidence_scores = []

        # Recommender scores
        top_blocks_indices, self.blocks_scores = assign_new_scores(
            N, k_similarities, 0.9, p_similarities, 0.1, confidence_scores)
